...
from convox.adapters.base import AdapterError, Clock, TrialContext
...
class RunOptions:
    ...
    #: Hold the floor for the estimated duration of each utterance. Disabled in
    #: unit tests so suites run fast; enabled everywhere else so that timing
...
async def run_trial(
    ...
    *,
    ...
    stream = AgentStream(session, recorder)
    stream.start()

    policy = ScriptedPolicy(scenario, persona, recorder, stream, realtime=options.realtime)
    status = TrialStatus.COMPLETED
    ended_by: EndedBy | None = None
    error: str | None = None

    try:
        ended_by = await asyncio.wait_for(policy.run(session), timeout=scenario.timeout_s)
    except TimeoutError:
